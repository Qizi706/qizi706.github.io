...
建议在写作期间保持 `draft: true`，完成后改成 `false`。可从 `/draft/` 查看草稿列表，
并通过 `/draft/<slug>/` 预览正文。草稿页会生成到生产站点，但带有 `noindex, nofollow`，
且不会出现在正式文章列表、分类、RSS 或 sitemap 中。这只是未收录入口，不是权限控制。
...
| `tags`        |  否  | 标签数组                                       |
| `draft`       |  否  | 默认 `false`；设为 `true` 时仅在 `/draft` 预览 |
