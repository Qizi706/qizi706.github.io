...
description: '用 USYSCALL 加速 getpid，打印多级页表并实现 pgaccess，结合分配与释放路径理解 RISC-V 页表权限、映射和访问位。'
pubDate: '2026-03-08T23:30:03+08:00'
updatedDate: '2026-09-07T00:33:57+08:00'
readingOrder: 3
readingNote: '内存：页表映射与访问权限'
categories:
  - '6.s081'
tags:
  - 'OS'
...
- **课程对照：** [MIT 6.S081 2021 · pgtbl](https://pdos.csail.mit.edu/6.828/2021/labs/pgtbl.html)，RISC-V xv6，课程实验分支 `pgtbl`。这里指定的是读者核对任务和接口的参考版本，文章发布日期是个人学习记录的日期。
- **本文范围：** USYSCALL、vmprint、pgaccess。系列的 Lab1—Lab9 是本博客的编号，以任务名称和课程分支定位源码。
- **历史证据：** 本文未附当时个人作答仓库的 commit SHA 和完整评分日志，不能据此恢复完全相同的代码快照。已有实现说明与踩坑记录保留，下面的命令供重新验证时使用，不代表本次修订重新运行了 xv6 实验。

在自己的 `pgtbl` 作答工作目录中，先保存版本和工作区状态，再运行该版本提供的评分器：
...
![图3-1：RISC-V simplified logical page table](/assets/6-s081-lab3_assert/simplified_logical_page_table.png)
...
![图3-2：RISC-V address translation details](/assets/6-s081-lab3_assert/address_translation_details.png)

![图3-3：RISC-V physical address details](/assets/6-s081-lab3_assert/physical_address_details.png)

### 二、内核地址空间

图3-4展示了内核虚拟地址如何映射为物理地址：

![图3-4：On the left, xv6’s kernel address space. On the right, the RISC-V physical address space that xv6 expects to see.](/assets/6-s081-lab3_assert/kernel_address_space_and_physical_address_space.png)
...
![图3-5：A process’s user address space, with its initial stack.](/assets/6-s081-lab3_assert/a_process_user_address_space_with_initial_stack.png)
