...
description: "作为 xv6 的最后一个核心大实验，Lab 9 (mmap) 标着硬核的 \"Hard\" 难度。但从本质上讲，它其实是 Lab 3（缺页中断）和 Lab 8（文件系统）的融合。 mmap 和 munmap 系统调用允许 UNIX 程序对自己的地址空间进行极其细致的控制。通过将磁盘文件直接映射到内存中，程序可以像读写..."
