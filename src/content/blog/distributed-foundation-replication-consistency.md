---
title: '分布式基础：副本和一致性'
description: '比较同步与异步复制、线性一致性、会话保证和 Quorum，区分 KV 数据与元数据的正确性要求，并分析旧副本和主副本切换风险。'
pubDate: '2026-07-14T10:30:00+08:00'
updatedDate: '2026-09-07T00:33:57+08:00'
readingOrder: 5
readingNote: '副本：确定读写正确性要求'
categories:
  - '分布式'
tags:
  - '副本'
...
> 系列入口：[分布式阅读路径](/category/分布式/)。 前置：[状态机、分片、路由与元数据](/blog/distributed-foundation-state-machine-sharding-metadata/)。 下一篇：[故障处理](/blog/distributed-foundation-failure-handling/)。

## 本文要解决的问题

同一份数据有多个副本后，读者需要知道“写成功”到底承诺了什么。本文比较复制方式与读写保证，再区分 KV 数据、owner、epoch 和统计字段：副本暂时缺失能否通过重算补救，与读到错误版本是否会破坏请求正确性，是两个需要分别回答的问题。
...
本文在单对象读写场景中，用“强一致性”具体指线性一致性：操作表现得像在调用与返回之间某个瞬间生效，并尊重实际发生的先后关系。具体 API 是否提供该保证，需要核对契约，例如 [etcd 的线性化与串行化读选项](https://etcd.io/docs/v3.6/learning/api_guarantees/)。

更直观地说：

```text
一次写入返回成功后才发起的读，应看到这次写入或更晚的写入；这不包括与写入重叠、尚未完成的读。
```
...
若没有后续更新，此次写入成功返回后发起的线性化查询应看到 node_b 和 epoch=11。
```
...
在固定的 N 个副本集合内，读集合与写集合一定相交。但集合相交本身不能证明线性一致性；还要规定如何比较版本、处理并发写和未完成写，以及故障切换时保留哪些提交。
...
这只是写入路径的轮廓。提交还必须服从所用共识协议的任期与日志安全规则，不能把“收到多数回复”直接等同于任意写入都可提交；可对照 [Raft 原论文 §5.4](https://raft.github.io/raft.pdf)。
...
```text
线性一致读：走协议规定的领导权确认、提交进度与读取流程。
允许旧值的读：读本地缓存或采用相应 follower 读选项。
...
这和[状态机与元数据篇](/blog/distributed-foundation-state-machine-sharding-metadata/)中限制旧 owner 写入的 fencing 是同一个思想。
...
假设 A 已提交 `owner=B, epoch=11`，只读副本仍保存 `owner=A, epoch=10`。分别分析“展示缓存位置”和“据此授权删除”这两个读用途，再说明读一致性选项、worker 端版本校验及重试策略。可容忍旧展示信息，不意味着可依据旧信息修改对象。

## 参考与对照

- [etcd API guarantees](https://etcd.io/docs/v3.6/learning/api_guarantees/)：对照线性化操作与允许旧值的读选项，避免笼统使用“强一致”这个标签。
- [Raft 原论文](https://raft.github.io/raft.pdf)：第 5.4 节讨论安全性和提交规则；多数副本确认不能脱离协议前提单独使用。
