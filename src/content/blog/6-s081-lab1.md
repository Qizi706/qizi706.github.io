...
description: '通过 sleep、pingpong、primes、find 和 xargs 熟悉 xv6 系统调用，解释管道 EOF、文件描述符生命周期及原始 fork 与 COW 的区别。'
pubDate: '2026-03-06T11:29:25+08:00'
updatedDate: '2026-09-07T00:33:57+08:00'
readingOrder: 1
readingNote: '入口：Unix 工具与系统调用使用'
categories:
  - '6.s081'
tags:
  - 'OS'
...
- **课程对照：** [MIT 6.S081 2021 · util](https://pdos.csail.mit.edu/6.828/2021/labs/util.html)，RISC-V xv6，课程实验分支 `util`。这里指定的是读者核对任务和接口的参考版本，文章发布日期是个人学习记录的日期。
- **本文范围：** sleep、pingpong、primes、find、xargs。系列的 Lab1—Lab9 是本博客的编号，以任务名称和课程分支定位源码。
- **历史证据：** 本文未附当时个人作答仓库的 commit SHA 和完整评分日志，不能据此恢复完全相同的代码快照。已有实现说明与踩坑记录保留，下面的命令供重新验证时使用，不代表本次修订重新运行了 xv6 实验。

在自己的 `util` 作答工作目录中，先保存版本和工作区状态，再运行该版本提供的评分器：
...
COW 也叫写时复制，是对 `fork` 的一种实现优化。原始 xv6 的 `fork` 会通过 `uvmcopy` 分配并复制父进程的用户内存；它并未预先实现 COW。

实现 COW 后，父子进程才能先共享物理页，通过只读映射和写入时的 page fault 延迟必要的复制。这样可以减少 `fork` 后立即 `exec` 等路径中的无效工作。具体修改见 [Lab5：Copy-on-Write Fork](/blog/6-s081-lab5/)，原始实现与实验目标可对照 [MIT COW 任务书](https://pdos.csail.mit.edu/6.828/2021/labs/cow.html)。
