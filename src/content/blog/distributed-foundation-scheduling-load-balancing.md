---
title: '分布式基础：调度和负载均衡'
description: '比较轮询、负载感知和局部性感知调度，结合 KV Cache 位置、请求阶段与背压，分析吞吐、排队延迟和迁移成本之间的取舍。'
pubDate: '2026-07-23T10:30:00+08:00'
updatedDate: '2026-09-07T00:33:57+08:00'
readingOrder: 8
readingNote: '调度：联合考虑计算、排队和局部性'
categories:
  - '分布式'
tags:
  - '调度'
...
> 系列入口：[分布式阅读路径](/category/分布式/)。 前置：[缓存策略、迁移与淘汰](/blog/distributed-foundation-cache-migration-eviction/)。

## 本文要解决的问题

一个空闲 worker 未必持有请求需要的 KV Cache。本文比较负载感知与局部性感知调度，把排队、数据搬运、重算和执行成本放到同一次决策中，再通过背压和资源隔离保护在线请求。策略是否有效，需要看目标负载下的吞吐与尾延迟。
...
设 A 排队更长但已有缓存，B 排队较短但需要搬运数据。分别估计 `排队 + 获取可用 KV + 执行` 的总成本，并标注 HBM 容量和请求 deadline 约束。改变 cache 大小与队列长度，找到选择发生变化的条件，再用测量检查估计误差。

## 参考与对照

- [Google SRE：Load Balancing in the Datacenter](https://sre.google/sre-book/load-balancing-datacenter/)：对照轮询、最少连接和加权负载选择的代价。
- [Google SRE：Handling Overload](https://sre.google/sre-book/handling-overload/)：对照排队限制与重试预算，理解负载均衡之外为什么还需要背压。
