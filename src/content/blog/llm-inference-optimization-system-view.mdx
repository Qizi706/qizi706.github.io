...
本文只建立机制模型。最小 Attention 实现、原始计时数据和实验失败记录见[《LLM 推理机制实验：KV Cache、预分配与 Batch Size》](/blog/vllm-inference-performance-lab/)；真实 vLLM Serving 是阶段 2 后半段的主要任务，不能用当前 Toy Lab 的结果替代。
...
本文负责解释为什么这些预测值得检验；[阶段 2 中间机制实验](/blog/vllm-inference-performance-lab/)负责记录当前 Toy Attention 支持了什么、证据边界在哪里，以及迁移到真实 vLLM 前还缺什么。最终 vLLM 性能文章只使用 S0–S4 真正产生的数据；必须保留原始 CSV/JSON、启动配置、版本、日志、内存记录、异常请求和复现命令，不能用理论预期替代尚未出现的结果。
