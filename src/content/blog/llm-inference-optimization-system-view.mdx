...
这篇文章只维护相对稳定的机制模型。一个月内的核心交付路线是 M2 → S0–S4 → F0；Mixed Prefill/Decode、Prefix Reuse、固定版本源码追踪与跨环境迁移作为进阶扩展，不阻塞阶段 2 的基础完成。完整步骤、命令、产物目录和验收条件统一保存在[阶段 2 学习执行手册](/labs/kv-cache-batch/docs/PLAN.md)。
...
- M0 已完成 Cache、预分配和固定 `B=2` 的 Batched Decode
- P0 已完成 NumPy View/Copy、Stride 与 Buffer 分配的语义闭环
- M1 已完成 `B=1/2/4/8` 扫描、240 条原始样本、曲线与 Knee 分析
- M2-A1 已用 Shape、元素映射和内存共享测试固定 Head 轴变换
- M2-A2 已完成 Q/K/V 投影、两个非零坐标 Oracle、五类非法配置和闭卷 Shape 推导；完整 12 项回归测试通过

## 当前任务

M2-B：先固定 `H_q=H_kv`，为四维 Q/K/V 建立无 Cache MHA Oracle。当前只验证 Score、Causal Mask、Softmax、`Weight @ V`、Future Token 不可见和 Batch/Head 隔离，不提前实现 Cache、GQA 映射或真实 Serving 实验。
...
M2-B 的预测、前置阅读、执行顺序与验收清单见 [M2 工作表](/labs/kv-cache-batch/docs/gates/M2.md)。未完成的 Gate 一律保持 `In progress` 或 `Pending`；理论预测不能代替原始数据、服务端指标或固定版本源码。
