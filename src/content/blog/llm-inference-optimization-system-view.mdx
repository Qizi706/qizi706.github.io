...
这篇文章只维护相对稳定的机制模型。一个月内的核心交付路线是 M2 → S0–S4 → F0；Mixed Prefill/Decode、Prefix Reuse、固定版本源码追踪与跨环境迁移作为进阶扩展，不阻塞阶段 2 的基础完成。完整步骤、命令、产物目录和验收条件统一保存在[阶段 2 学习执行手册](/labs/kv-cache-batch/view/docs/PLAN.md/)。
...
M2-A2：让 Q/K/V 共享一套可验证的投影与 Head Shape 契约。当前不提前实现 Score、Mask、Softmax，也不开始真实 Serving 实验。
...
A2 的预测、前置阅读、执行顺序与验收清单见 [M2 工作表](/labs/kv-cache-batch/view/docs/gates/M2.md/)。未完成的 Gate 一律保持 `In progress` 或 `Pending`；理论预测不能代替原始数据、服务端指标或固定版本源码。
