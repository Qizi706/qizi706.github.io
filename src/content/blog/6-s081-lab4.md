...
description: '通过 backtrace 与 alarm 梳理 RISC-V Trap、trampoline 和 trapframe，说明用户回调之后怎样恢复被中断的执行现场。'
pubDate: '2026-03-17T17:00:00+08:00'
updatedDate: '2026-09-07T00:33:57+08:00'
readingOrder: 4
readingNote: '异常：Trap 入口与现场恢复'
categories:
  - '6.s081'
tags:
  - 'OS'
...
- **课程对照：** [MIT 6.S081 2021 · traps](https://pdos.csail.mit.edu/6.828/2021/labs/traps.html)，RISC-V xv6，课程实验分支 `traps`。这里指定的是读者核对任务和接口的参考版本，文章发布日期是个人学习记录的日期。
- **本文范围：** backtrace、alarm / sigreturn。系列的 Lab1—Lab9 是本博客的编号，以任务名称和课程分支定位源码。
- **历史证据：** 本文未附当时个人作答仓库的 commit SHA 和完整评分日志，不能据此恢复完全相同的代码快照。已有实现说明与踩坑记录保留，下面的命令供重新验证时使用，不代表本次修订重新运行了 xv6 实验。

在自己的 `traps` 作答工作目录中，先保存版本和工作区状态，再运行该版本提供的评分器：
...
![图 4-1 stack frame](/assets/6-s081-lab4_assert/stack_frame.png)
