...
description: '实现用户级线程切换、并发哈希表与 barrier，结合 xv6 调度器理解上下文保存、锁保护和等待唤醒。'
pubDate: '2026-03-27T20:00:00+08:00'
updatedDate: '2026-09-07T00:33:57+08:00'
readingOrder: 6
readingNote: '执行：线程切换与同步'
categories:
  - '6.s081'
tags:
  - 'OS'
...
- **课程对照：** [MIT 6.S081 2021 · thread](https://pdos.csail.mit.edu/6.828/2021/labs/thread.html)，RISC-V xv6，课程实验分支 `thread`。这里指定的是读者核对任务和接口的参考版本，文章发布日期是个人学习记录的日期。
- **本文范围：** uthread、并发哈希表、barrier。系列的 Lab1—Lab9 是本博客的编号，以任务名称和课程分支定位源码。
- **历史证据：** 本文未附当时个人作答仓库的 commit SHA 和完整评分日志，不能据此恢复完全相同的代码快照。已有实现说明与踩坑记录保留，下面的命令供重新验证时使用，不代表本次修订重新运行了 xv6 实验。

在自己的 `thread` 作答工作目录中，先保存版本和工作区状态，再运行该版本提供的评分器：
...
\begin{aligned}
\text{old (Process A)}
  \to \text{current CPU scheduler process (Scheduler)}\to \text{new (Process B)}
\end{aligned}
$$

![图 6-1 Switching from one user process to another](../../assets/6-s081-lab6_assert/switching_from_one_user_process_to_another.png)
