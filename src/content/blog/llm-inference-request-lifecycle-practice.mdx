...
脚本保存在[实验产物目录](/labs/llm-inference-request-lifecycle/)中。需要注意：Core Version 被固定，而 Metal Plugin 通过 latest release 解析，因此它记录了当时的安装过程，却不是完全 Hermetic 的构建。
...
[Browse `/labs/llm-inference-request-lifecycle/`](/labs/llm-inference-request-lifecycle/) 可以在线查看或下载全部复现产物：
