...
description: '跟踪一个 BlockGroup 从创建到迁移、淘汰的生命周期，解释状态机、引用计数、分片、路由缓存与 epoch 怎样共同保护所有权。'
pubDate: '2026-07-07T16:30:00+08:00'
updatedDate: '2026-09-07T00:33:57+08:00'
readingOrder: 4
readingNote: '状态：明确对象归属和请求路由'
categories:
  - '分布式'
tags:
  - '状态机'
...
> 系列入口：[分布式阅读路径](/category/分布式/)。 前置：[RPC 和远程调用](/blog/distributed-foundation-rpc-remote-call/)。 下一篇：[副本和一致性](/blog/distributed-foundation-replication-consistency/)。

## 本文要解决的问题

一个 BlockGroup 从创建到淘汰，需要同时回答它在哪里、归谁管理、现在允许什么操作。本文把状态机与元数据放在同一条生命周期里，重点分析迁移过程中旧路由、并发访问和 epoch 校验怎样配合。状态名是这里的建模示例，落到实际系统时需要核对其契约。
...
画出 A→B 的迁移时间线：复制、目标校验、提交新 location/epoch、发布路由、释放源副本。把一个携带旧 epoch 的读请求插入每个间隙，逐一写出由谁接受、拒绝或引导刷新，以及源副本何时才满足释放条件。

## 参考与对照

- [etcd API guarantees](https://etcd.io/docs/v3.6/learning/api_guarantees/)：对照事务、线性化读写和 revision 的契约；路由缓存与 Watch 需要单独处理滞后。
- [Raft 原论文](https://raft.github.io/raft.pdf)：对照复制状态机的日志顺序与安全性，理解元数据共识层提供的保证。
