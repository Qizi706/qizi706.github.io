...
如果每次加法都得到正确同步，结果应为 `200000`。但上面的程序对普通 `int` 进行了没有同步的并发读写，存在 **data race，并因此产生未定义行为（undefined behavior）**；C++ 标准不保证它的最终输出。

为了理解丢失更新，可以把 `++counter` 示意性地拆成：
...
两个线程的读写交错可以解释一次“丢失更新”，但这个三步示意不是对未定义行为程序所有执行结果的穷举。编译器也不必保留这种逐步执行方式。定义依据见 [C++ 标准草案：Data races](https://eel.is/c++draft/intro.races)。
...
如果只是简单计数，可以使用 `std::atomic<int>` 的原子递增操作：
...
`std::atomic<int>` 的 `++counter` 是原子的读—改—写操作，对这个计数器的更新不会以“先读、后写”的两个独立原子操作暴露给其他线程。它不表示操作系统不能调度其他线程，也不保证所有原子类型的实现都是无锁的。递增与 `fetch_add` 的契约见 [C++ 标准草案：原子整数类型](https://eel.is/c++draft/atomics.types.int)。
...
原子对象的单次 load、store 和其提供的读—改—写操作各有原子性保证；但把多个原子操作组合起来，并不会自动得到一个原子事务。例如下面的 `count = count + 1` 是一次原子读取加一次原子写入：
...
在 `foo` 函数中，每次循环先读取 `count` 的值，然后再 `+1` 赋值回去，虽然读取和赋值都是原子操作，整个序列却不具备原子性。这里可以发生逻辑上的竞态并丢失更新，但不属于前面普通 `int` 示例的 data race，也不能据此说原子访问本身导致未定义行为。需要整体递增时，应改用 `++count` 或 `count.fetch_add(1)`。
