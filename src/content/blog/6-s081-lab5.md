...
description: '为 xv6 fork 实现写时复制，串联物理页引用计数、uvmcopy、usertrap 与 copyout，解释共享只读页何时需要真正复制。'
pubDate: '2026-03-20T20:00:00+08:00'
updatedDate: '2026-09-07T00:33:57+08:00'
readingOrder: 5
readingNote: '扩展：在页表和异常机制上实现 COW'
categories:
  - '6.s081'
tags:
  - 'OS'
...
- **课程对照：** [MIT 6.S081 2021 · cow](https://pdos.csail.mit.edu/6.828/2021/labs/cow.html)，RISC-V xv6，课程实验分支 `cow`。这里指定的是读者核对任务和接口的参考版本，文章发布日期是个人学习记录的日期。
- **本文范围：** 引用计数、COW 页处理、copyout。系列的 Lab1—Lab9 是本博客的编号，以任务名称和课程分支定位源码。
- **历史证据：** 本文未附当时个人作答仓库的 commit SHA 和完整评分日志，不能据此恢复完全相同的代码快照。已有实现说明与踩坑记录保留，下面的命令供重新验证时使用，不代表本次修订重新运行了 xv6 实验。

在自己的 `cow` 作答工作目录中，先保存版本和工作区状态，再运行该版本提供的评分器：
...
COW 还应在 `make qemu` 启动的 xv6 shell 中运行 `cowtest` 和 `usertests`，两者覆盖范围不同；判定标准见上方课程任务书。
...
![copyout 处理 COW 页的流程：循环检查地址和页表，处理 COW 缺页后验证写权限并复制数据](../../assets/6-s081-lab5_assert/copyout-cow-flow.svg)

_图 5-1 ：Copyout COW Flow_
