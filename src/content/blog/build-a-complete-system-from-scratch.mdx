...
updatedDate: '2026-08-20T02:48:37+08:00'
categories:
  - '随笔'
tags:
  - '软件工程'
...
kv-cache-batch/
├── pyproject.toml
├── src/kv_cache_batch/attention.py
...
```bash
mkdir -p kv-cache-batch/src/kv_cache_batch kv-cache-batch/tests
cd kv-cache-batch

$EDITOR pyproject.toml
$EDITOR src/kv_cache_batch/attention.py
...
当前项目正在进入 M2。已有的 `attention.py` 只有单 Head；下一步需要解释 MHA/GQA 中 `H_q`、`H_kv` 与 KV Cache 容量的关系。
...
等 Head 轴真的可靠，再迁移到 Q/K/V、Cached MHA、GQA 映射和容量公式。模块的名字和边界由已经理解的机制决定，而不是由想象中的最终 Transformer 决定。
...
只阅读当前步骤所需的最小材料，写下运行前预测，然后实现一条能被测试的路径。当前的 [M2 工作表](/labs/kv-cache-batch/view/docs/M2_NOTES.md/) 就只做 `split_heads` 和两个测试，不顺手完成整个 Multi-Head Attention。
...
<CodeCard title="一次 120 分钟练习应该留下什么？" path="public/labs/kv-cache-batch/docs/M2_NOTES.md">

```text
Question:
  怎样证明 Head 轴被正确拆出，而不只是 Shape 碰巧正确？

Prediction:
  q[b,h,t,d] == projected[b,t,h*D_head+d]

Action:
  实现 split_heads；写 Shape 与元素映射测试

Feedback:
  unittest 的第一个失败 + 手算坐标
...
```text
填写 Shape 与元素映射预测
  -> 实现 split_heads(projected, num_heads)
  -> 写两个最小测试
  -> 运行构建命令
  -> 关闭代码重新实现
```

之后才依次进入 Q/K/V、Cached MHA、GQA 和 KV 容量；M2 通过后停止扩展 Toy Transformer，进入真实 vLLM 环境、负载、调度、流式请求、Prefix Reuse、源码追踪和复现交付。
