...
之后才依次进入无 Cache MHA、Cached MHA、GQA 和 KV 容量；M2 通过后停止扩展 Toy Transformer。阶段 2 的一个月核心交付只继续完成真实 vLLM 环境、稳态基线、输入长度、并发、调度预算和最终性能报告；Mixed Prefill/Decode、Prefix Reuse、源码追踪与跨环境迁移保留为进阶扩展，不再阻塞原定阶段产出。
