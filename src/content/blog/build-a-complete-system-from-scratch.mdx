...
以我现在的 [`p2-inference-systems` Lab](/labs/p2-inference-systems/) 为例。最初要回答的问题并不是“如何设计一个 AI Infra 平台”，而是：
...
只阅读当前步骤所需的最小材料，写下运行前预测，然后实现一条能被测试的路径。[M2-A1](/labs/p2-inference-systems/read/checkoffs/m2-a1-split-heads/) 与 [M2-A2](/labs/p2-inference-systems/read/checkoffs/m2-a2-qkv-projection/) 工作表已经保存 Head 轴与 Q/K/V 投影证据；当前 B 只增加无 Cache Score、Mask、Softmax、逐 Head Output 和隔离性测试，不顺手完成 Cached MHA 或 GQA。
...
<CodeCard title="一次 120 分钟练习应该留下什么？" path="labs/p2-inference-systems/checkoffs/m2-b-causal-attention.md">
...
Artifact:
  multi_head_attention.py、test_multi_head_attention.py、checkoffs/m2-b-causal-attention.md
