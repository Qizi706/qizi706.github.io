...
当前项目正在推进 M2。已有的 `attention.py` 仍是单 Head；M2-A1 已经把 Head 轴从扁平特征中可靠地拆出，M2-A2 又把这套规则迁移到 Q/K/V，并增加输入契约。A2 定向 6 项、完整 12 项回归测试和闭卷 Shape 推导均已通过。
...
Head 轴有了 A1 的测试保护后，M2-A2 把同一规则迁移到 Q/K/V，并在 `reshape` 前检查输入维度、Head 整除关系和一致的 `D_head`。当前 M2-B 才开始在 `H_q=H_kv` 下实现无 Cache MHA；之后依次进入 Cached MHA、GQA 映射和容量公式。模块的名字和边界由已经理解的机制决定，而不是由想象中的最终 Transformer 决定。
...
只阅读当前步骤所需的最小材料，写下运行前预测，然后实现一条能被测试的路径。[M2-A1](/labs/kv-cache-batch/read/docs/gates/m2/a1/) 与 [M2-A2](/labs/kv-cache-batch/read/docs/gates/m2/a2/) 工作表已经保存 Head 轴与 Q/K/V 投影证据；当前 B 只增加无 Cache Score、Mask、Softmax、逐 Head Output 和隔离性测试，不顺手完成 Cached MHA 或 GQA。
...
<CodeCard title="一次 120 分钟练习应该留下什么？" path="labs/kv-cache-batch/docs/gates/m2/b.md">

```text
Question:
  Q/K/V 已是 [B,H,T,D_head] 时，怎样证明 Score、Mask、Softmax
  没有混合 Batch、Head 或 Future Token？

Prediction:
  B=2,H=3,T=4,D_head=5 时，Score/Weight=[2,3,4,4]
  逐 Head Output=[2,3,4,5]，Future Token 不改变历史输出

Pre-read:
  matmul stacked matrices + swapaxes + triu（15 min）

Action:
  先写独立标量 Oracle、Causal 与隔离性测试；再实现无 Cache MHA

Feedback:
  unittest 的第一个失败 + 手算 Score/Output 坐标 + 完整回归

Artifact:
  multi_head.py、test_multi_head.py、docs/gates/m2/b.md

Transfer:
  关闭代码后重做推导；下一轮进入 Cached MHA
```
...
```text
填写 Score、Weight、Output Shape、Softmax Axis 与隔离性预测
  -> 先写标量 Oracle、Future Token 与 Head/Batch 隔离测试
  -> 实现 H_q=H_kv 的无 Cache MHA
  -> 运行定向测试和完整回归
  -> 关闭代码重新推导并解释缩放与归一化轴
```

之后才依次进入 Cached MHA、GQA 和 KV 容量；M2 通过后停止扩展 Toy Transformer。阶段 2 的一个月核心交付只继续完成真实 vLLM 环境、稳态基线、输入长度、并发、调度预算和最终性能报告；Mixed Prefill/Decode、Prefix Reuse、源码追踪与跨环境迁移保留为进阶扩展，不再阻塞原定阶段产出。
