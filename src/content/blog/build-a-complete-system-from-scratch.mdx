...
只阅读当前步骤所需的最小材料，写下运行前预测，然后实现一条能被测试的路径。M2-A1 与 M2-A2 的私人工作表已经保存 Head 轴与 Q/K/V 投影证据；当前 B 只增加无 Cache Score、Mask、Softmax、逐 Head Output 和隔离性测试，不顺手完成 Cached MHA 或 GQA。
