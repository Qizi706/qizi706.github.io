...
readingOrder: 4
readingNote: '机制实验：继续验证 KV Cache 与固定 Batch'
categories:
  - 'AI Infra'
tags:
  - 'LLM'
...
机制背景见[《LLM 推理优化的系统视角：从 KV Cache 到调度》](/draft/llm-inference-optimization-system-view/)。

[`p2-inference-systems` 公开材料目录](/labs/p2-inference-systems/)提供当前 Starter 及其运行文件、当前与历史任务书、空白工作表和当前评分器。个人实现、已填写工作表，以及仍保留的原始样本、汇总和图表由私人作答仓库管理，不随网站发布。本文公开实验结果与解释，当前下载包不包含完整的 M0/M1 复现材料。
...
因此，下面的表格是**历史结果记录**，不是逐样本证据完整的一组可复核测量。以后重跑产生的数据必须作为新实验保存，不能冒充原始样本。
...
**H2 的当前可复现性：`Inconclusive`。** 在重新生成独立的 <code>results/cache-strategy/raw.csv</code> 之前，这个趋势不能从本文保留的汇总记录重新聚合、检查分布或核对测量协议。新的复跑只能形成新的证据版本，不能补写成当时的原始数据。
...
以下保留 M0/M1 的复跑命令。执行前需要取得对应版本的实验代码和依赖环境，并进入保存这些代码的私人作答仓库根目录；当前公开 Starter 不包含这些 Benchmark CLI 和结果目录，不能直接运行下面的流程。当前练习的启动方式以[公开 Lab 使用说明](/labs/p2-inference-systems/read/README/)为准。

<CoreCard title="历史命令：需对应实验代码与环境" class="title-underline">
...
下面是本文实验记录涉及的目录结构，用于说明私人实验材料的组织方式，不是当前公开下载包的文件清单。公开内容以 [`p2-inference-systems` 材料目录](/labs/p2-inference-systems/)为准；Cache Strategy 缺失的原始样本不在下列已记录产物中。
