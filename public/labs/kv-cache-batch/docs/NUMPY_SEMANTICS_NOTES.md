...
实验文件：`examples/numpy_array_semantics.py`。每个 ndarray Case 都通过
...
- [x] 六个 Case 都保留了原始预测或明确标记“未记录”。
- [x] 六个 Case 都定义了可观察结果和断言。
- [x] 至少一个预测错误得到解释和修正。
- [x] 已区分名称绑定、浅复制、View、Copy、Stride 和数据 Buffer 分配。
