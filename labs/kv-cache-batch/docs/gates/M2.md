...
定位：这是 M2 的**填写与证据记录页**，用于保存运行前预测、第一次失败、修正规则、验收结果和下一决策。当前任务与评分命令以 [`README.md`](../../README.md) 为准；M2 在整个阶段中的依赖与停止条件见 [`docs/PLAN.md`](../PLAN.md)。

这份工作表把 M2 拆成可以独立完成的小闭环，但不作为第二份任务说明。不要一次实现完整 MHA、GQA 和 KV Cache；每次只增加一个可以被测试否证的变换。
