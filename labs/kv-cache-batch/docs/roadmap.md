...
| 执行当前练习             | [`assignments/m2-b.md`](../assignments/m2-b.md) | 不展开完整阶段路线和历史实验         |
| 查看依赖、验收和停止条件 | 本计划                                          | 不重复当前练习的命令、提示和填写记录 |
| 填写预测、失败与结果     | [`docs/gates/m2/b.md`](gates/m2/b.md)           | 不定义阶段范围，也不充当教程         |
...
预测、首次失败、修正规则与闭卷验收保存在 [M2-A1 工作表](gates/m2/a1.md)。A1 已关闭，不再把后续 Q/K/V 投影计入 A1 完成状态。
...
- 运行后结果、首次失败、修正规则与闭卷推导已回写 [M2-A2 工作表](gates/m2/a2.md)。
...
预测、120 分钟执行顺序与运行记录填写在 [M2-B 工作表](gates/m2/b.md)。全部通过后进入 M2-C；任一项失败时只修正 M2-B。
