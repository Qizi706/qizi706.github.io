...
| 怎样审计详细阶段边界？         | [`docs/roadmap.md`](../docs/roadmap.md)       | 完整实验协议、阶段验收与停止条件         |
| 我的预测、失败和结果记在哪里？ | [`docs/gates/m2/b.md`](../docs/gates/m2/b.md) | 边做边填写的工作记录，不是第二份任务说明 |
...
这里保留 A1/A2 只是为了说明 M2-B 的可信起点；完整预测、失败与验收历史分别保存在 [M2-A1](../docs/gates/m2/a1.md) 和 [M2-A2](../docs/gates/m2/a2.md) 工作表中。
...
先关闭实现文件，在 [`docs/gates/m2/b.md`](../docs/gates/m2/b.md) 中写出 Score、Mask、Weight、Output 的 Shape 和 Softmax Axis。然后在 `tests/test_multi_head.py` 中写一个不调用生产函数的标量 Oracle，保存第一次失败，再完成 `TODO`。
...
<p>最终评分必须是 <code>Score: 100/100</code>。然后回到 <a href="/labs/kv-cache-batch/read/docs/gates/m2/b/">M2-B 工作表</a>填写第一次失败、修正规则与闭卷解释；只有验收清单全部完成，才进入 M2-C Cached MHA。</p>
</div>
