...
os.environ.setdefault(
    "MPLCONFIGDIR",
    str(Path(tempfile.gettempdir()) / "p2-inference-systems-matplotlib"),
)
...
def write_plots(
    ...
    *,
    ...
    matplotlib.rcParams["svg.hashsalt"] = "p2-inference-systems-m1-batch-size-scan"
    ...
    latency_figure.savefig(
        output_dir / "latency-p50-p95.svg",
        format="svg",
        metadata={"Date": None},
    )
    ...
    throughput_figure.savefig(
        output_dir / "throughput-positions-per-second.svg",
        format="svg",
        metadata={"Date": None},
    )
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--input",
        type=Path,
        default=Path("results/m1-batch-size-scan/raw.csv"),
    )
