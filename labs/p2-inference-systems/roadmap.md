...
这份文件是 2026.08.12—2026.09.12 阶段 2 的范围、依赖和验收标准，不是当前练习的逐步操作说明。跨 Phase 1/2 的入口见 [AI Infra 实验路线](/learning/)；可视化的 Phase 2 Gate 顺序见[阶段 2 路线页](/learning/phase-2/)。目标是在一个月内理解推理系统的核心矛盾，并用真实 vLLM 实验解释调度、显存、Batching 与 KV Cache 如何共同决定吞吐和延迟。
...
| 需要                     | 唯一入口                                                                       | 这份材料不负责什么                   |
| ------------------------ | ------------------------------------------------------------------------------ | ------------------------------------ |
| 查看当前 Gate 和入口     | [阶段 2 路线页](/learning/phase-2/)                                            | 不保存实现细节和原始证据             |
| 执行当前练习             | [`assignments/m2-b-causal-attention.md`](assignments/m2-b-causal-attention.md) | 不展开完整阶段路线和历史实验         |
| 查看依赖、验收和停止条件 | 本计划                                                                         | 不重复当前练习的命令、提示和填写记录 |
| 填写预测、失败与结果     | [`checkoffs/m2-b-causal-attention.md`](checkoffs/m2-b-causal-attention.md)     | 不定义阶段范围，也不充当教程         |
...
完整记录见 [`checkoffs/p0-numpy-semantics.md`](checkoffs/p0-numpy-semantics.md)，可执行例子见 [`examples/p0_numpy_array_semantics.py`](examples/p0_numpy_array_semantics.py)。P0 已关闭，不再追加脱离当前主线的 Python 练习。
...
M0 依次建立 Full Recompute、Dynamic KV Cache、Preallocated KV Cache 和固定 `B=2` Batched Decode，并用 Full Recompute 作为逐位置正确性 Oracle。直接产物包括实现、测试与 [`results/m0-length-scan/raw.csv`](results/m0-length-scan/raw.csv) 中的 300 条长度扫描样本。
...
运行前预测保存在 [`checkoffs/m1-batch-size-scan.md`](checkoffs/m1-batch-size-scan.md)。当时冻结的内容是：
...
Batch Size Scan 不能覆盖 `results/m0-length-scan/raw.csv`；每个实验必须拥有独立的结果目录。
...
```bash
rg -c '^batch_size_scan,' results/m1-batch-size-scan/raw.csv
```
...
产物：[`results/m1-batch-size-scan/summary.csv`](results/m1-batch-size-scan/summary.csv)。所有字段由 `p2-inference-systems-summary` 从 `raw.csv` 计算。
...
预测、首次失败、修正规则与闭卷验收保存在 [M2-A1 工作表](checkoffs/m2-a1-split-heads.md)。A1 已关闭，不再把后续 Q/K/V 投影计入 A1 完成状态。
...
- 运行后结果、首次失败、修正规则与闭卷推导已回写 [M2-A2 工作表](checkoffs/m2-a2-qkv-projection.md)。
...
预测、120 分钟执行顺序与运行记录填写在 [M2-B 工作表](checkoffs/m2-b-causal-attention.md)。全部通过后进入 M2-C；任一项失败时只修正 M2-B。
