...
| `assignments/<gate>.md`           | 当前及历史任务怎样执行和评分   | 不重复完整阶段路线 |
| 公开 `checkoffs/`                 | 当前及历史任务的未填写模板     | 不保存维护者答案   |
...
- `usage` 驱动网站 Lab 首页：`evidence` 展示复现包用法，`starter` 额外声明当前任务、当前空白工作表、公开源码与私人仓库边界。
- `checks.required` 声明构建前必须存在的证据。
- `checks.csvRows` 固定关键原始数据的行数。
- 路线、当前状态和页面展示数据统一放在 `src/data/lab-roadmap/`，禁止写入 `lab.json`。
- 公开 `src/` 可以作为未完成 Starter；维护者的实现、已填写工作表和结果必须放入被忽略且具有独立 Git 历史的 `.work/` 仓库。
- `task-archive` Starter 可以发布当前与历史任务书，但所有公开工作表都必须登记不可变哈希，且不能发布任何 `results/` 文件；构建门禁会强制检查这一点。
